...
def feeds(request):
    ...
    toRead = list(Subscription.objects.filter(
        Q(user=request.user) & (Q(is_river=True) | Q(last_read__lt=F('source__max_index')))).order_by("source__name"))
...
def addfeed(request):
    try:
        ...
        if request.method == 'GET':
            ...
            headers = {"User-Agent": "FeedThing/3.3 (+http://%s; Initial Feed Crawler)" % request.META["HTTP_HOST"],
                       "Cache-Control": "no-cache,max-age=0",
                       "Pragma": "no-cache"}  # identify ourselves and also stop our requests getting picked up by google's cache
                ...
                for l in soup.findAll(name='link'):
                    if l.has_attr("rel") and l.has_attr("type"):
                        if l['rel'][0] == "alternate" and (
                                l['type'] == 'application/atom+xml' or l['type'] == 'application/rss+xml' or l[
                            ...
                            rethtml += '<li><form method="post" onsubmit="return false;"> <input type="hidden" name="feed" id="feed-%d" value="%s"><a href="#" onclick="addFeed(%d)" class="btn btn-xs btn-default">Subscribe</a> - %s</form></li>' % (
                                feedcount, urljoin(feed, l['href']), feedcount, name)
                            feed = urljoin(feed, l[
                                'href'])  # store this in case there is only one feed and we wind up importing it
...
def readfeed(request, fid):
    ...
    if sub.source == None:
        ...
        if not sub.is_river:
            ...
            for src in sources:
                srcposts = list(
                    Post.objects.filter(Q(source=src.source) &
                                        Q(index__gt=src.last_read)).order_by("index"))
        ...
        if not sub.is_river:
            posts = list(Post.objects.filter(Q(source=sub.source) &
                                             Q(index__gt=sub.last_read)).order_by("index"))
            posts_per_page = feed_posts_per_page
